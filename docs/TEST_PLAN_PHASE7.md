# 阶段七：测试与优化 - 测试计划

**创建日期**: 2026-01-11  
**状态**: 📝 计划中

## 📋 测试任务清单

### ✅ 已完成的测试

1. **阶段一：模板系统测试**
   - 文件: `tests/test_template_system.py`
   - 状态: ✅ 已完成（4/4测试通过）

2. **阶段二：RAG 增强检索测试**
   - 文件: `tests/test_rag_retrieval.py`
   - 状态: ✅ 已完成（5/5测试通过）

3. **阶段三：权限处理测试**
   - 文件: `tests/test_permission_phase3.py`
   - 状态: ✅ 已完成（6/6测试通过）

### 📝 待创建的测试

1. **阶段四：图表生成测试**
   - 文件: `tests/test_chart_phase4.py`
   - 测试项:
     - 文件存在性验证
     - 代码结构验证（chart_generator 函数）
     - LangGraph 集成验证
     - AgentState 状态定义验证
     - PromptBuilder 方法验证

2. **阶段五：数据源选择与推荐问题测试**
   - 数据源选择测试: `tests/test_datasource_phase5.py`
     - 文件存在性验证
     - 代码结构验证（datasource_selector 函数）
     - LangGraph 集成验证
     - AgentState 状态定义验证
   - 推荐问题测试: `tests/test_question_phase5.py`
     - 文件存在性验证
     - 代码结构验证（question_recommender 函数）
     - LangGraph 集成验证

3. **阶段六：动态 SQL 测试（可选）**
   - 文件: `tests/test_dynamic_phase6.py`
   - 测试项:
     - 文件存在性验证
     - 代码结构验证（dynamic_sql_generator 函数）
     - PromptBuilder 方法验证

4. **集成测试**
   - 文件: `tests/test_integration_phase7.py`
   - 测试项:
     - 完整流程测试（从用户查询到结果返回）
     - 各节点集成验证
     - 状态流转验证

## 💡 执行顺序

1. ✅ 创建阶段四测试
2. ✅ 创建阶段五测试
3. ✅ 创建阶段六测试（可选）
4. ✅ 创建集成测试
5. 📝 文档更新

## 📊 测试覆盖率目标

- 单元测试覆盖率: > 80%
- 集成测试覆盖率: > 60%
- 代码结构测试: 100%

## ⚠️ 注意事项

- 代码结构测试不依赖运行时环境（sqlalchemy, pgvector等）
- 实际运行时测试需要完整的数据库和依赖环境
- 集成测试需要模拟或真实的数据库连接

---

**下一步**: 开始创建阶段四和阶段五的测试文件
