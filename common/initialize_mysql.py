...
def execute_user_qa_record_sql(***REMOVED***:
    ...
    to4_answer,qa_type,file_key,create_time***REMOVED*** VALUES(%s, %s, %s,%s, %s, %s, %s, %s, %s, %s,%s,%s***REMOVED***
    """
    # 紧凑格式数据，去掉换行
    data_to_insert = [
        (
        ...
        ***REMOVED***,
        (
            ...
            r'\n\n从数据来看，各分局的案件总数呈现出明显的区域差异。**徐汇分局以10起案件位居首位**，其次是**浦东分局（7起）和朝阳、天河、海淀分局（均为4起）**，其他分局案件数均低于3起。  \n\n**关键发现**  \n- 徐汇分局案件数量最多，是最低值的10倍  \n- 三个分局并列第二梯队，案件数为4起  \n- 超过半数分局案件数≤2起  \n\n**注意**  \n该数据未体现时间维度变化，仅反映静态总量分布情况，建议补充时间序列数据以分析动态趋势。"***REMOVED***, "dataType": "t02"***REMOVED***',
            r'{"data": {"chart_type": "柱状图", "template_code": "temp03", "data": [["product", "总数"], ["浦东分局", "7"], '
            r'["朝阳分局", "4"], ["天河分局", "4"], ["南山分局", "3"], ["武侯分局", "1"], ["锦江分局", "2"], ["徐汇分局", "10"], ["海淀分局", "4"], ["白云分局", "3"], ["福田分局", "2"]], "note": "数据来源: xxx数据库，以上数据仅供参考，具体情况可能会根据xx进一步调查和统计而有所变化"***REMOVED***, "dataType": "t04"***REMOVED***',
            "DATABASE_QA",
            "",
            "2025-07-09 14:36:41",
        ***REMOVED***,
